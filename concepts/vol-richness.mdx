...
The row is **two-tailed**, and the tails mean different things. A record-**wide** spread flags **amber** — fragile calm, a conditional warning: nothing has broken, but the index's stillness depends entirely on correlation staying pinned. A **compressed** spread flags **red** — stocks and index moving as one is the correlation-stress signature, what crashes actually look like. Mirror-image of the implied-correlation row's coloring, so the two framings always agree.
