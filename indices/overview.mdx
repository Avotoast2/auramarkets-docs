...
Up top, one chart overlays your chosen indices as **% change from the window's start** — S&P vs NASDAQ 100 vs Russell vs Dow out of the box, with the **+** picker to add any index, sector, or industry group to the mix.

It runs on the same engine as the [stock pop-out](/indices/stock-chart), so all the TradingView-style moves work here too:

- **Hover** — crosshair with a per-index readout, each value colored green/red
