...
<Note>
The **Volatility** and **Market Breadth** panels each have a **gear icon** in their header — click it to customize which metrics (including the breadth cockpit’s five dimensions) show. The **World Indices** panel has one gear of its own: pick **one to three price columns** from presets (the usual timeframes) or a **custom date range**, and manage region membership in the same editor. And the page-header **Layout** button goes further — [remove and rearrange entire sections](/getting-started/customize).
</Note>

## The verdict panels

Three panels between the index map and the movers read the market's internals and commit to a call:

- **Market Breadth** — the five-dimension breadth cockpit: **Participation** (% above the 50-day), **Direction** (McClellan oscillator), **Extremes** (net new 52-week highs/lows), **Leadership**, and **Conviction** (net 9:1 up vs down days). Each row carries its percentile vs its own history plus a sparkline, and the panel synthesizes one state — **HEALTHY / THRUST / WASHOUT / DIVERGENCE / NARROW** — with a one-line read of what's driving it. The **Decompose →** link opens the **Breadth Explorer**: the same breadth split by sector, size, theme, or basket, with each group's contribution to the move and a two-year breadth-vs-own-history quilt — the "who's actually washed out" view. → [Breadth, explained](/concepts/breadth)
- **Volatility** — the vol regime verdict (**Panic / Underpriced risk / Rich premium / Cheap protection / Normal**) built on the IV−RV premium spine. → [The vol regime, explained](/concepts/vol-richness)
- **Credit** — the [Credit Monitor](/macro/credit)'s verdict strip: state, key spreads, and tripwire count at a glance, clicking through to the full monitor.

Rows in the breadth and volatility panels are charts in disguise — **click any dimension** to open it full-size with the standard [chart engine](/indices/stock-chart) moves (timeframes, studies, compare).

## Top Stories

A curated headline panel fed by a multi-outlet newswire — Bloomberg's feeds plus WSJ, CNBC, MarketWatch, FT, and NYT — deduplicated across sources, filtered to market-relevant stories, and refreshed around the clock. Each row shows the outlet tag and timestamp; a **×3**-style count marks stories corroborated by multiple outlets, and when a story is directly about a listed name, its **ticker chip** opens the [stock pop-out](/indices/stock-chart). The same wire feeds the [Today page's Wire panel](/getting-started/today), the Market Pulse, the Daily Recap's session tape, and the overnight section of the [Pre-Market Brief](/macro/daily-digest).
...
The legend below doubles as the scoreboard — each index's current window % lives next to its name, and clicking a legend entry toggles that line on or off. Hovering a legend entry also reveals an **×** that removes the series from the chart entirely (it goes back to the picker, and the removal sticks to your saved layout) — toggle for a quick look, × when you're done with a line.

Each line also carries a **right-edge tag** — name plus the current window's % move, sign-colored to match the legend — so on a crowded multi-line chart you can tell who's who at the right margin without hunting the legend. Tags track whatever window you've zoomed to.
