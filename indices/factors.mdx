...
During market hours a **“Today so far” strip** sits above the table: the current books repriced live every 15 minutes (15-minute-delayed quotes), one chip per basket — the L/S spread in progress, long baskets vs SPY. Deliberately raw moves only, no σ or p: a half-finished day isn’t drawn from the distribution of completed sessions the scoreboard’s stats are measured against. The strip disappears outside trading hours; the official daily print still lands after the close.

One row per basket × 1D / 5D / 1M / 3M / 6M / 1Y / calendar-YTD, plus three stat columns: **Sharpe** (trailing-1Y annualized return ÷ vol), **MaxDD** (worst peak-to-trough inside the trailing year), and **Vol** (current 21-day realized vol, annualized — the "trading fat or calm?" read). Each stat is ranked against that basket's own rolling history of the same stat, so their σ·p lines mean the same thing the return ones do. There's deliberately no since-inception column — the backfill's start date is a data-provider artifact, not an economic anchor. Every cell carries the return (or vol level) plus **σ and p against that series' own overlapping history** — fat-tail honest, exactly the math the pulse detector uses. Cells read like `−3.3σ · p0` — see [σ and p notation](/concepts/notation). Cells at |σ| ≥ 2 get an amber flag. A whole row lighting up is a de-grossing or squeeze day.

The panel shares its row with the Performance chart, so the stat columns sit behind a horizontal scroll on most screens — the **Expand →** link in the header stacks the layout and gives the table the full page width, all ten columns at once. **Collapse ←** restores the side-by-side view, and the choice is remembered.
...
A second tab that answers two questions the Overview can't: *what are these baskets actually made of*, and *how much of a theme's move is really its own story*?

### Composition

One block per basket: stacked **sector bars for the long and short legs** (count-share of the current members), the **drift vs six rebalances ago**, and the basket's top **theme-coverage chips** (which thematic-taxonomy themes its members cluster in — Momentum's long book reading `Semiconductors 18% · Commodities Metals 15%` tells you what the momentum trade *is* right now). A **Corpus Lens** strip up top shows how your writers' coverage tilts across the books.

When an extreme basket's top sector crosses ~40% of the book, The Read on the Overview tab gains a **Makeup** line — "this extreme is concentrated sector risk, not diversified factor risk."

### Story premium

Every theme's equal-weight daily returns regressed on **SPY + the three L/S factor spreads** over the trailing six months. The table splits each theme's 21-day and 63-day return into the **factor-explained part** and the **residual** — the residual is what the theme's own story earned after stripping out market beta and factor exposure. `Electric Vehicles +17.2% actual = −6.9% factor + 24.1% story` is a genuinely idiosyncratic theme move; a theme whose gain is all factor is just the momentum trade wearing a costume. The same decomposition rides the [thematic heatmap's](/indices/thematic-heatmap) tile tooltips, along with each theme's factor-book fingerprint.
...
<Note>
Honest residual limits: pre-2026 rebalances still screen survivors with today's market caps (dead names use adjusted close × last-known shares); Most Shorted can't recover dead names (their FINRA short-interest history wasn't archived); dead-company filing coverage for Non-Profitable Tech is patchy. Official series are daily — the intraday “Today so far” strip is a live estimate of the day in progress, not part of the recorded history. Most-Shorted updates on FINRA's bi-monthly publication cycle, same as the bank versions. Still a live regime gauge first, a backtest second.
</Note>
