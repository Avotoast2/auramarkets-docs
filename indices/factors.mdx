...
<Note>
Honest limits: the universe is today's constituents, so the ~4-year backfill carries survivorship bias — and it compounds the further back you look (the momentum short leg especially: names that went to zero aren't in it). Treat the series as a live regime gauge, not an academic backtest, and don't read the earliest segment as one at all. Series are daily (no intraday). Most-Shorted updates on FINRA's bi-monthly publication cycle, same as the bank versions.
</Note>
