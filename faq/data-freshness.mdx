...
## Earnings season vs history

**Source:** FactSet Earnings Insight (the free weekly S&P 500 earnings scorecard).

**Cadence:** weekly during earnings season — new editions typically publish Fridays, and the strip picks them up within hours. FactSet skips some weeks (the strip's source tag always shows which edition it's reading). Quarterly history runs back to 2019.

## Fund flows

**Source:** issuer daily records (iShares, State Street, VanEck, Invesco) for exact shares outstanding and NAV; FactSet-derived series for funds whose issuers don't publish daily counts; Farside Investors for crypto ETFs.

**Cadence:** issuers post files with the prior session's data, typically overnight — the flows page reflects them within hours. Historical depth varies by fund (back to 2006 for SPDR funds, to inception for most iShares funds).
