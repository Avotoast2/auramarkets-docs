...
At the top sits the market's current state, presented as a **captioned state strip** — the regime labeled as the *multi-day backdrop*, with the day score on its band gauge beside it — the same component the [Daily Recap](/macro/daily-recap#the-state-strip) hero uses, so "the regime says Risk On but the score says cautious" reads as two different clocks, not a contradiction. (A **Recap badge** also appears here once the Daily Recap publishes after 4:30 PM ET.) This is **not AI sentiment** — it's a deterministic engine that reads only prices and classifies the tape into one of six states:
...
- **Raw Feed** — chronological list of recent updates: news from the deduplicated multi-source wire (last 8 hours, timestamped ET — the same curated feed the [Newswire](/indices/overview#top-stories) runs on), model alerts, regime flips. Good for the moment-by-moment.
...
- **Factor-basket extremes** — auramarkets' in-house [momentum / high-beta / low-vol L/S spreads and squeeze baskets](/indices/overview#the-factor-baskets) moving ≥2σ against their own history, reported with their σ · p line and empirical rank ("worst 1M run on record") — the de-grossing/squeeze-day detector. During market hours the factor input also carries a **“today so far”** line — the books repriced live — so intraday commentary describes the day in progress instead of narrating yesterday’s close
