...
The hero disambiguates three constructs that live on different clocks:

- **Today's session** — the intraday *shape* classifier (Range-Bound, Trending, Reversal…): how the day traded.
- **Backdrop (multi-day)** — the regime engine's standing state (Risk On, Risk Off, Rotational…): the environment the day traded *in*. A range-bound session inside a risk-on regime is not a contradiction — they answer different questions.
- **Day score** — the quant score's net of the day's signals, shown on a **band gauge** (bearish / cautious / mixed / cautious / bullish at −30/−10/+10/+30) so a reading like *+15 · cautious* is self-explanatory: mildly positive, not yet bullish. A relationship sentence underneath ties all three together.

The same strip appears in the Market Pulse sidebar and on [Today](/getting-started/today)'s regime card, so the three concepts read identically everywhere.

## Evidence is clickable

Tickers in the recap prose (SPY, QQQ, a single name with its move) open the standard [stock pop-out](/indices/stock-chart) in place, and **entity references link to their source surface**: a theme like *Gold Miners* opens its chart, an AI value-chain segment like *Memory/Storage* lands on [AI Watch](/indices/ai-watch)'s chain table with that segment expanded, and metric phrases (DDR5 spot, hyperscaler G-spread, RPO) jump to the exact panel that tracks them. Only verified references link — ambiguous words stay plain text. Hovering a linked ticker briefly previews a mini price chart.
...
- **Claude** writes the prose by reading those features plus the day's quant timeline, the Pulse events, historical context for similar days, and a computed **AI-complex evidence block** from [AI Watch](/indices/ai-watch) — group moves, value-chain segment outliers, the CDS gap, hyperscaler bond spreads, hardware spot — so on days the AI trade drives the tape, the recap can attribute the move ("Memory −3% with DDR5 spot rolling over") instead of hand-waving "semis weak".
- The AI never invents numbers — it only narrates the structured data. Sections stay in their lane by rule: a rates or credit driver appears in an equities read only when tied to named equity moves, and a theme already covered by another section is referenced, never restated.
